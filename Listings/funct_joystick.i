...
# 2 "Source/joystick/funct_joystick.c" 2
...
# 3 "Source/joystick/funct_joystick.c" 2
    ...
    for (i = 0; i < 4; i++) {
        for (j = 0; j < 4; j++) {
            if (current_tetronomis[i][j] != 0) {
                ...
                if (y >= 0 && y < 20 && placed_color[y][x] != 0) {
                    return 0;
                }
            }
        }
...
void move_left(void) {
    if (allow_to_move_left()) {
        draw_rotate_tetronomis(current_x, current_y, 0x0000);
        current_x--;
