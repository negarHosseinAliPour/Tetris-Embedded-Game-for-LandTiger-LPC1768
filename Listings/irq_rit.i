...
extern volatile int bg_active; // used by RIT
# 26 "Source/RIT/IRQ_RIT.c" 2
...
extern int allow_to_move_down(void);
extern void freeze_shape(void);
...
void RIT_IRQHandler(void)
{
  ((LPC_SC_TypeDef *) ((0x40080000UL) + 0x7C000) )->EXTINT |= (1 << 1);
    ...
    if (gamestarted && !isPaused) {
        ...
        // -------- Read ADC and compute base falling ticks --------
        int adc_val = AD_current; // 0..4095

        // Map ADC to ticks: 1 sq/sec = 20 ticks, 5 sq/sec = 4 ticks
        int base_ticks = 20 - ((adc_val * 16) / 4095); // 20..4
        if (base_ticks < 4) base_ticks = 4;
        if (base_ticks > 20) base_ticks = 20;



    if (slowdown_active && base_ticks < 20) {

      base_ticks = 20; // force 1 square/second
      slowdown_ticks++;

      if (slowdown_ticks >= 300) { // 15 seconds (300 * 50ms)
        slowdown_active = 0;
        slowdown_ticks = 0;

      }
    }
        // -------- Soft drop: DOUBLE current speed (halve ticks) --------
        int effective_ticks = base_ticks;

        // Joystick DOWN on P1.26 (active low)
        if ((((LPC_GPIO_TypeDef *) ((0x2009C000UL) + 0x00020) )->FIOPIN & (1 << 26)) == 0) {
            effective_ticks = base_ticks / 2;
            if (effective_ticks < 1) effective_ticks = 1; // safety
        }

        // -------- Auto-fall timing --------
        down_counter++;
        if (down_counter >= effective_ticks) {
            down_counter = 0;

            if (allow_to_move_down()) {
                move_down();
            } else {
                // Piece can’t fall: lock it and spawn next
                freeze_shape();
                new_shape = 1;
            }
        }

        // -------- Left movement (P1.27 active low) --------
        if ((((LPC_GPIO_TypeDef *) ((0x2009C000UL) + 0x00020) )->FIOPIN & (1 << 27)) == 0) {
            left_counter++;
            if (left_counter >= 5//5 //for left and right movement) {
                move_left();
                left_counter = 0;
            }
        } else {
        ...
        // -------- Right movement (P1.28 active low) --------
        if ((((LPC_GPIO_TypeDef *) ((0x2009C000UL) + 0x00020) )->FIOPIN & (1 << 28)) == 0) {
            right_counter++;
            if (right_counter >= 5//5 //for left and right movement) {
                move_right();
                right_counter = 0;
            }
        } else {
        ...
        // -------- Rotation (P1.29 active low, one-shot) --------
   ...
   if (bg_active && !isNotePlaying()) {
     ...
     if (bg_active) {
       ...
       if (currentNote == (sizeof(song)/sizeof(song[0]))) {
         currentNote = 0; // loop (or disable_RIT() if you want stop)
       }
     }
}
