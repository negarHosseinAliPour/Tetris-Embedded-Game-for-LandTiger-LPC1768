...
// if (key1_debounce == 0) {
// key1_debounce = 10; // Wait 500ms before next KEY1 can be accepted
// if (gamestarted == 0) {
// new_game();
// gamestarted = 1;
// isPaused = 0;
...
//
// GUI_Text(50, 10, (uint8_t *)"                   ", 0x0000, 0x0000);//clear the press key text
// } else {
// isPaused = !isPaused; //oggle the pause

// if (isPaused) { //for pausing the game
// GUI_Text(170, 10, (uint8_t *)"Pause ", 0xF800, 0x0000);
// } else { //resuming the game
// GUI_Text(170, 10, (uint8_t *)"         ", 0x0000, 0x0000);

// }
// }
//
// // debounce timer = 20 ticks (20 * 50ms = 1 second)
// key1_debounce = 1; //1 if you use emulator 20 if you use real board
// }
//
// ((LPC_SC_TypeDef *) ((0x40080000UL) + 0x7C000) )->EXTINT |= (1 << 1); // Clear pending interrupt
//}
void EINT1_IRQHandler(void) {
    // Check if the debounce timer has expired
    if (key1_debounce == 0) {

        // 1. Set debounce FIRST to a safe value (e.g., 10 ticks = ~500ms)
        // Use 1 for simulator, 10-20 for real board
        key1_debounce = 10;
        ...
        if (gamestarted == 0) {
            ...
            // Note: It is better to move GUI_Text out of ISR, but we leave it for now
            GUI_Text(50, 10, (uint8_t *)"                    ", 0x0000, 0x0000);
        } else {
            // ... (Pause Logic) ...
        ...
        // REMOVED THE LINE: key1_debounce = 1;
        // We want to keep the value '10' we set at the top.
    }
...
void EINT2_IRQHandler (void)
{
 ...
 if(gamestarted && !isPaused){
  ...
  while(allow_to_move_down()){ //if key2 pressed and shape can move down-> move down to the end
  move_down();
  }
 }
